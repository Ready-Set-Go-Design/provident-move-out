...
function FormPage1() {
  ...
  const [rentingAnnounceKey, setRentingAnnounceKey] = useState<number>(0);
  const location = useLocation();
  const urlParams = new URLSearchParams(location.search);
  const from = urlParams.get("from");
  const formErrorId = "form-errors";
  const sellingRadioId = "selling-radio";
  ...
  useEffect(() => {
    if (formData.selling_or_renting === "renting") {
      setRentingAnnounceKey((current) => current + 1);
    }
  }, [formData.selling_or_renting]);
  ...
  const focusFirstInvalid = () => {
    ...
    if (
      ...
      const radio = document.getElementById(sellingRadioId);
    ...
    if (formData.moving_day === "") {
      document.getElementById("moving-day")?.focus();
      return;
    }

    if (formData.moving_month === "") {
      document.getElementById("moving-month")?.focus();
      return;
    }

    if (formData.moving_year === "") {
      document.getElementById("moving-year")?.focus();
    }
  };
